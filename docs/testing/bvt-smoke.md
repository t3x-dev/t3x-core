...
| 启动命令 | `cd t3x-webui && npm run dev` |
| 测试命令 | `cd t3x-webui && npm test -- src/__tests__/integration/` |
...
### Precondition
- 服务启动成功（`GET /api/v1/health` 返回 `status: ok`）
...
```
 ✓ src/__tests__/integration/bvt.conversation.test.ts (4 tests) 2869ms
 ✓ src/__tests__/integration/bvt.readpaths.test.ts (4 tests) 3014ms
 ✓ src/__tests__/integration/bvt.commit.test.ts (4 tests) 3067ms
 ✓ src/__tests__/integration/bvt.turn.test.ts (5 tests) 3191ms
...
```
t3x-webui/src/__tests__/integration/
├── helpers/
│   ├── client.ts             # API 客户端封装
│   ├── hash.ts               # Hash 重算验证（王炸断言）
│   └── index.ts
├── bvt.turn.test.ts          # BVT-1
├── bvt.commit.test.ts        # BVT-2
├── bvt.conversation.test.ts  # BVT-3
└── bvt.readpaths.test.ts     # BVT-4
```
