...
> **ContextFlow is "Git for Meaning"** — enabling any conversation or discussion to be versioned, archived, verified, and shared like code.
>
...
|-------|------|----------------|---------------|
| **Bottom: Framework Core** | `contextflow-core` | Deterministic extraction + evidence + storage/versioning: hash, extract findings, persist turns (SQLite+JSONL), diff, merge, commit, .cfpack export | ❌ No |
| **Middle: Agentic Layer** | `contextflow-agents` | Pluggable SummaryAgent / MergeAgent plugins (can use LLM or pure rules) | ✅ Optional |
| **Top: Product Layer** | `contextflow.app` | CLI + WebUI for interaction, visualization & sharing | ❌ (optional enhancement) |
...
- Core algorithms & components:
  ...
  - **Storage**: all turns / conversations / drafts / commits / merges / diffs append to JSONL ledgers under `.contextflow/` as the semantic journal, with SQLite kept in sync as a rebuildable query index.
...
> - Plugins are wired via `.contextflow/config.yml` (`extractors`, `sentence_encoder`, `evidence_scoring`). Internally the SDK exposes `register_extractor/register_embedder/register_agent` so custom code can register implementations before runtime.
...
ContextFlow persists data in two layers:

1. **JSONL Ledger (source of truth):** Every verifiable state (turn chains, commit chains, etc.) is written as JSON Lines under `.contextflow/`, normalized via JCS and hashed with SHA-256 to form append-only chains.
...
The ledger is split by entity into multiple JSONL streams under `.contextflow/` (see `STORAGE_ARCHITECTURE.md` for paths). Each row is hashed after JCS normalization.
...
|--------|-----------|----------------|--------------|-------|
| Conversation store | Core | ✅ | ✅ | Semantic data contract is fixed; storage engines can swap (`.contextflow` JSONL, SQLite, Postgres, etc.) as long as they follow `schema/` and `STORAGE_ARCHITECTURE.md`. |
...
`.cfpack` is ContextFlow’s semantic archive / interoperability format. It packages complete semantic history for transfer between implementations; it is not the runtime storage itself but an export/import contract.
...
- **Design constraints**:
  ...
  - As an open format, third-party systems just need `.cfpack` read/write support to interoperate with ContextFlow, independent of this repository’s implementation.
...
| `contextflow-agents` | ✅ (default templates) | ❌ |
| `contextflow.app` (WebUI) | Partially open | ✅ SaaS model |
| `contextflow.cloud` (Hub) | ❌ | ✅ Private semantic repository + Collaboration |
...
| **Public Release** | 2026.Q2 | ContextFlow Hub, multi-user collaboration, plugin marketplace |
...
### ContextFlow is Git for Meaning:
...
│                     PRODUCT LAYER                           │
│  ┌────────────────┐  ┌────────────────┐  ┌────────────────┐ │
│  │  CLI interface │  │  WebUI interface│  │ ContextFlow Hub│ │
...
| **ContextFlow** | **Semantic versioning** | **❌ Core (Agentic can use LLM optionally)** | **✅** | **Deterministic core + pluggable LLMs** |
