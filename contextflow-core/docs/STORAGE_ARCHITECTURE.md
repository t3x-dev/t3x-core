...
ContextFlow's storage is divided into two layers:

1. **JSONL Primary Ledger (Ledger)**: All auditable, reproducible semantic state (Turn chain, Commit chain, etc.) is written as JSON Lines under the `.contextflow/` directory, using JCS canonicalization + SHA-256 hashing to form an append-only hash chain.
...
The primary ledger is split into multiple JSONL streams by entity, located under `.contextflow/` (actual path is defined in `STORAGE_ARCHITECTURE.md`). Each line is a record, hashed after JCS canonicalization.
