 ...
 * Ported from Python: contextflow-core/core/extractors/polarity_rules.py
 *
 * Rules:
 ...
 */
export class PolarityRuleEngine {
  private readonly positiveVerbs: Map<string, PolarityRule>;
  private readonly negativeVerbs: Map<string, PolarityRule>;

  constructor(customRules?: { positive?: PolarityRule[]; negative?: PolarityRule[] }) {
    const rules = {
   ...
   */
  private hasNegation(verbToken: NLPToken, tokens: NLPToken[]): boolean {
  ...
  extractPreferenceRelations(tokens: NLPToken[]): PreferenceRelation[] {
    const relations: PreferenceRelation[] = [];
   ...
   */
  getPolarityVerbs(): { positive: string[]; negative: string[] } {
    return {
