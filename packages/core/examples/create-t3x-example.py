#!/usr/bin/env python3
"""
Example: Creating T3X files from scratch

This script shows different ways to craft T3X artifacts locally without
...
def create_minimal_t3x() -> None:
    """Create the absolute minimal valid T3X file."""
    artifact = {
        "t3x_version": "1.0",
        "metadata": {
    ...
    with open("minimal.t3x", "w", encoding="utf-8") as handle:
        json.dump(artifact, handle, indent=2)

    print("✓ Created minimal.t3x")


def create_project_context_t3x() -> None:
    """Create a project-scoped T3X artifact with richer content."""
    artifact = {
        "t3x_version": "1.0",
    ...
    with open("project-context.t3x", "w", encoding="utf-8") as handle:
        json.dump(artifact, handle, indent=2)

    print("✓ Created project-context.t3x")


def create_with_sdk() -> None:
    """Create a T3X artifact using the Python SDK."""
    try:
        from t3x import (
            T3XFile,
            T3XMetadata,
    ...
    save("learning-context.t3x", artifact)
    print("✓ Created learning-context.t3x (using SDK)")
...
def import_from_chatgpt() -> None:
    ...
    with open("chatgpt-imported.t3x", "w", encoding="utf-8") as handle:
        json.dump(artifact, handle, indent=2)

    print("✓ Created chatgpt-imported.t3x")


if __name__ == "__main__":
    print("Creating T3X files...\n")

    create_minimal_t3x()
    create_project_context_t3x()
    create_with_sdk()
    import_from_chatgpt()

    print("\n✓ All examples created!")
    print("\nValidate them with:")
    print("  t3x validate minimal.t3x")
    print("  t3x validate project-context.t3x")
