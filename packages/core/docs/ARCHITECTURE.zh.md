...
> **T3X 是“semantic版 Git”** —— 让任何conversation/讨论都能像代码一样被版本化,归档,校验与共享.
>
...
| 层级 | 名称 | 职责 | 依赖 LLM 吗? |
|------|------|------|----------------|
| **底层:Framework Core** | `t3x-core` | 决定论抽取 + evidence + 存储/版本:hash,抽取 findings,持久化 turn(SQLite+JSONL),diff,merge,commit,`.cfpack` export | ❌ 不依赖 |
| **中层:Agentic Layer** | `t3x-agents` | 可插拔 SummaryAgent / MergeAgent(可接 LLM 或纯规then) | ✅ optional |
| **顶层:Product Layer** | `t3x.app` | CLI + WebUI(由外部project实现,本仓库仅提供接口)用于交互,可视化与共享 | ❌(可加成) |
...
义),Ring 1 在此阶段就output词形归一 + polarity
  ...
  - **存储**:所有 turn / conversation / draft / commit / merge / diff write `.t3x/` 下的 JSONL 作为semanticledger,同时同步到 SQLite 作为queryIndexes;SQLite 可随时从 JSONL 重建
...
所有具有审计意义,必须可复现的status(例如 Turn 链,Commit DAG)都会以 JSON Lines 形式write .t3x/ directory下的若干file中.
...
> **补充description(2025-11)**:步骤 3/5 以及 Must/Mustn’t 列表management由 Core 决定论执行;步骤 4/6 由 Agentic SummaryAgent 负责(桥接模板 + LLM 生成).Bridge YAML 模板位于 `.t3x/bridges/`,CLI 初始化时writedefault版本,Agentic 直接read该directory.
...
CLI 由外部project实现,用户进入 T3X 会话后会通过 `/command`(如 `/draft`,`/commit`,`/diff`,`/merge` 等)触发核心能力.本仓库仅提供 Draft / Commit / Diff / Merge / `.cfpack` 等决定论 API,CLI 的界面与command解析逻辑均由外部project负责.
...
| 模块 | 所属层 | 决定论? | 可替换? | 备注 |
|------|--------|----------|----------|------|
| Conversation store | Core | ✅ | ✅ | semantic数据契约固定,存储引擎可换(`.t3x` JSONL / SQLite 等符合 `schema/` 与 `STORAGE_ARCHITECTURE.md`) |
...
`.cfpack` 是 T3X 的semantic归档 / 互operations格式,用于在不同实现之间交换完整的semantic版本历史.它不是运行时唯一的数据结构,而是面向导入/export,备份与审计的开放协议.
...
- **设计约束**:
  ...
  - 作为开放格式,第三方系统只需实现 `.cfpack` 读写即可接入 T3X 生态,与本仓库实现解耦.
...
| `t3x-agents` | ✅(default模板) | ❌ |
| `t3x.app`(WebUI) | 部分开源 | ✅ SaaS 模式 |
| `t3x.cloud`(Hub) | ❌ | ✅ 私有semantic仓库 + 协作 |
...
| **T3X** | **semantic版控** | **❌ 核心(Agentic 层optional接 LLM)** | **✅** | **决定论 Core + LLM 可插拔** |
