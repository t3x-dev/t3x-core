...
describe('collectLessons', () => {
  ...
  it('returns empty array when no failed assertions', () => {
    const leaves = [
      {
        assertions: [{ id: 'ast_1', constraint_id: 'cst_1', passed: true, details: 'ok' }],
      },
    ];
  ...
  it('skips assertions without lesson field', () => {
    const leaves = [
      {
        assertions: [{ id: 'ast_1', constraint_id: 'cst_1', passed: false, details: 'failed' }],
      },
    ];
    expect(collectLessons(leaves)).toEqual([]);
  });
  ...
  it('deduplicates lessons', () => {
    const leaves = [
      {
        assertions: [
          { id: 'ast_1', constraint_id: 'c1', passed: false, details: '', lesson: 'Same lesson' },
        ],
      },
      {
        assertions: [
          { id: 'ast_2', constraint_id: 'c2', passed: false, details: '', lesson: 'Same lesson' },
        ],
      },
    ];
  ...
  it('handles leaves with no assertions', () => {
    const leaves = [{ assertions: undefined as any }, { assertions: [] }];
    expect(collectLessons(leaves)).toEqual([]);
  });
});
