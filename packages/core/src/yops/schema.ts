...
const SetOpSchema = z.object({
  set: z.object({
    path: z.string().min(1).describe('Node path + slot key separated by / (e.g. trip/budget, trip/dining/cuisine)'),
    value: SlotValueSchema.describe('The value — string, number, boolean, array, or nested object'),
    source: z.string().min(1).describe('Verbatim key phrase from the conversation that contains this fact'),
  ...
  }).strict().describe('Set or update a slot value on an existing node. Use when a fact is stated or changed in conversation.'),
}).strict();

const UnsetOpSchema = z.object({
  unset: z.object({
    path: z.string().min(1).describe('Node path + slot key to remove (e.g. trip/budget)'),
  }).strict().describe('Remove a slot from a node. Use when a fact is retracted or no longer relevant.'),
}).strict();
...
const AddOpSchema = z.object({
  add: z.object({
    ...
    source: z.record(z.string(), z.string()).describe('Per-slot source quotes from the conversation'),
    from: z.string().min(1).describe('Turn reference (e.g. T1)'),
    confidence: z.number().min(0).max(1).optional().describe('Extraction confidence 0-1'),
  }).strict().describe('Create a new node with initial slots. Use when a new topic or subtopic is introduced.'),
}).strict();
...
const DropOpSchema = z.object({
  ...
  }).strict().describe('Remove a node and all its children. Use when a topic is deleted or contradicted entirely.'),
}).strict();
...
const RenameOpSchema = z.object({
  ...
  }).strict().describe('Change a node key name. Use when the name is wrong or needs a better label.'),
}).strict();
...
const CloneOpSchema = z.object({
  ...
  }).strict().describe('Duplicate a node for variant exploration.'),
}).strict();

const MoveOpSchema = z.object({
  move: z.object({
    path: z.string().min(1).describe('Source node path'),
    to: z.string().min(1).describe('Full target path including new key (e.g. trip/hotel moves hotel under trip)'),
  }).strict().describe('Move a node to a different parent. Use when a node is under the wrong parent.'),
}).strict();
...
const NestOpSchema = z.object({
  ...
  }).strict().describe('Group sibling nodes under a new wrapper node. Use when related siblings should be organized together.'),
}).strict();
...
const SplitOpSchema = z.object({
  ...
  }).strict().describe('Break a node slots into child nodes. Use when a node has too many slots covering different sub-topics.'),
}).strict();

const FoldOpSchema = z.object({
  fold: z.object({
    path: z.string().min(1).describe('Path of the wrapper node to remove'),
  }).strict().describe('Remove a wrapper node, promote its only child. Use when a node has exactly 1 child and no slots.'),
}).strict();
...
const MergeOpSchema = z.object({
  ...
  }).strict().describe('Combine sibling nodes into one. Use when siblings overlap or should be consolidated.'),
}).strict();
...
const RelateOpSchema = z.object({
  relate: z.object({
    ...
    type: RelationTypeSchema.describe('Relation type: causes, conditions, contrasts, follows, depends'),
    confidence: z.number().min(0).max(1).optional().describe('Relation confidence 0-1'),
  }).strict().describe('Add a semantic relation between two nodes. Use when nodes have a causal, conditional, or dependency relationship.'),
}).strict();
...
const UnrelateOpSchema = z.object({
  ...
  }).strict().describe('Remove a semantic relation between two nodes.'),
}).strict();
