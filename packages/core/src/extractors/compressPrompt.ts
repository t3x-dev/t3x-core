...
const COMPRESS_SYSTEM_PROMPT = `You are a semantic frame compressor. Given a set of extracted frames with their engagement signals, produce YOps that merge redundant frames and remove low-value content.
...
2. COMPRESS FIRST:
   - Redundant frames with no user engagement (mention_count = 1, no manual edit) → merge
   - Frames not recently referenced (high last_touched) → remove
3. COMPRESS LAST:
   - Protected frames that are highly redundant with each other → merge (preserve all slots)
4. When merging, combine slot values from both frames (no information loss)
5. Output 'remove' and 'update' actions only (no 'add', 'define', or 'populate')
6. Relations use field names 'from' and 'to' (NOT 'source'/'target')
7. If there is nothing worth compressing, return an empty changes array
