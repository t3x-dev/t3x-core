...
describe('parseIncrementalResponse', () => {
  ...
  it('filters out proposals with missing required fields', () => {
    const raw = JSON.stringify([
      { type: 'new', text: 'Valid', confidence: 0.9, inference_type: 'direct', reasoning: 'r', evidence: [{ conversation_id: 'c', turn_hash: 'h', quoted_text: 'q', role: 'primary', relevance: 'r' }] },
  ...
  it('handles modify proposal with target_sp_id', () => {
    const raw = JSON.stringify([{
      ...
      evidence: [{ conversation_id: 'c', turn_hash: 'h', quoted_text: 'q', role: 'primary', relevance: 'r' }],
    }]);
    const result = parseIncrementalResponse(raw);
    expect(result).toHaveLength(1);
