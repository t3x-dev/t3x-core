...
export function buildSemanticGatePrompt(
  ...
  const systemPrompt = `You are a semantic extraction reviewer. You receive a source conversation and the Tree structure extracted from it.

Score the extraction from 0 to 1 on the five dimensions below and list issues:

## 1. Completeness
Were important intents, decisions, facts, and constraints from the conversation extracted?
- Check: every substantive statement has a matching tree node or slot
- Do not require: greetings, repetition, or process chatter

## 2. Accuracy
Do extracted slot values match the source text?
- Check: numbers, names, and dates are exact
- Check: inferred content has an explicit source citation

## 3. Relations
Are relationship types between trees correct?
- causes: does A actually cause B?
- conditions: is A actually a precondition of B?
- contrasts: do A and B actually conflict?
- follows: does A actually happen before B?
- depends: does A actually depend on B?

## 4. Granularity
- Over-split: one intent broken into unnecessary tree nodes?
- Over-merged: distinct intents stuffed into one tree node?

## 5. Hallucination
- Does any tree content never appear in the source?
- Are inferences reasonable, or overreaching?
...
\`\`\``;
  ...
  const userPrompt = `Source conversation:
${turnsText}
...
Return a 0-1 score for each dimension plus a concrete issue list.`;
...
export function buildCoveragePrompt(
  ...
  const systemPrompt = `You are a semantic extraction coverage reviewer. You receive a source conversation and the Tree structure extracted from it.

Your task: check whether important information in the source conversation is missing from every tree node.

## Criteria
- Important: intents, decisions, facts, constraints, numbers, times, names, and concrete needs
- Unimportant (ignore): greetings, repetition, filler, and process chatter (for example "ok", "sure", "let me think")
...
- coverage_ratio: 0-1, share of important information that is covered
- uncovered_segments: important source fragments that are not covered (quote the source)
- If everything is covered, return coverage_ratio: 1.0 and uncovered_segments: []`;
...
Judge coverage and return JSON.`;
