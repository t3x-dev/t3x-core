...
type PostureContext = Parameters<TransitionNativeStatementProvider['verify']>[0];
type GenerationAssessmentContext = Omit<PostureContext, 'effect'> & { operations: ProtocolValue[] };
async function assessGeneration(
  context: GenerationAssessmentContext,
  options?: { supportVerifier?: ProposalGenerationSupportVerifier }
): ReturnType<TransitionNativeStatementProvider['verify']> {
  const preparation = parseProposalGenerationPreparation(context.preparationFacts);
  const verifier = independentVerifier(options?.supportVerifier, preparation);
...
export function createProposalGenerationPostureProvider(options?: {
  ...
  return {
    ...
    async verify(context) {
      if (!isRecord(context.preparationFacts)) return { outcome: 'not_applicable' };
      if (context.preparationFacts.schema === 't3x.dev/proposal-generation-preparation/v1')
        return assessGeneration({ ...context, operations: context.effect.operations }, options);
      const checks = await authoringGenerationChecks(context);
      if (checks === null || checks.length === 0) return { outcome: 'not_applicable' };
      const predicates = [];
      for (const check of checks) {
        const assessed = await assessGeneration({ ...context, ...check }, options);
        if (assessed.outcome !== 'statement') throw new TypeError('Missing generation assessment');
        predicates.push(parseRunnerValidationStatement(assessed.statement).predicate);
      }
      const findings = predicates.flatMap((predicate) => predicate.findings);
      return {
        outcome: 'statement',
        statement: buildRunnerValidationStatement({
          state: context.result,
          ...
          predicate: {
            ...
            workflow: PROPOSAL_POSTURE_VERIFIER_WORKFLOW,
            environment: PROPOSAL_POSTURE_VERIFIER_ENVIRONMENT,
            inputManifest: authoringPreparationResource(
              context.preparationFacts,
              context.projectId,
              context.transitionId
            ),
            inputArtifacts: [
              ...new Map(
                predicates
                  .flatMap((predicate) => predicate.inputArtifacts)
                  .map((artifact) => [canonicalizeProtocolValue(artifact), artifact])
              ).values(),
            ].sort(compareResource),
            logs: [],
            outputs: [],
            outcome: predicates.every((predicate) => predicate.outcome === 'passed')
              ? 'passed'
              : 'failed',
            summary: `Verified ${checks.length} generated authoring action(s) at their exact historical inputs.`,
            findings,
          },
        }),
      };
