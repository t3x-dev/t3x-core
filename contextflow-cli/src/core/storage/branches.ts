...
export function createBranch(input: CreateBranchInput): BranchRecord {
  const db = getDb();
  ...
  const existingBranches = db
    .prepare(`SELECT COUNT(*) as c FROM branches WHERE project_id = ?`)
...
export function getBranch(project_id: string, name: string): BranchRecord | null {
  const db = getDb();
  const row = db
...
export function getBranchById(branch_id: string): BranchRecord | null {
  const db = getDb();
  const row = db
...
export function listBranches(options: ListBranchesOptions): BranchRecord[] {
  const db = getDb();
...
export function getCurrentBranch(project_id: string): BranchRecord | null {
  const db = getDb();
  const row = db
...
export function switchBranch(project_id: string, branch_name: string): BranchRecord | null {
  const db = getDb();

  const branch = getBranch(project_id, branch_name);
  ...
  return getBranch(project_id, branch_name);
}
...
export function deleteBranch(project_id: string, branch_name: string): boolean {
  const db = getDb();
...
export function ensureMainBranch(project_id: string): BranchRecord {
  const existing = getBranch(project_id, 'main');
