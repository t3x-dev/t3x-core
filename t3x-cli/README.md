...
T3X CLI provides a minimal viable terminal chat environment: simply run `t3x` at the command line to enter an interactive Shell, configure/view Claude API Key, select model, have ongoing conversations with Sonnet 4.5, and write each conversation turn to a JSONL file under `.t3x/`.
...
3. (Optional) Link CLI to global PATH:
   ...
   npm link
   ```
   After completion, you can run `t3x` from any directory.
...
On startup, it automatically detects/creates `.t3x` directory and displays the current conversation log path. The shell defaults to "chat mode" where you can directly input content to converse with the model; you can also adjust behavior through slash commands.
...
When starting `t3x`, you'll first be asked for your current VPN/proxy address (e.g. `127.0.0.1:10808`):
...
All configurations are written to `~/.config/t3x/config.json` and take effect immediately at runtime.

## Conversation Logs and Memory
- Each time CLI starts, it defaults to using the `default` project and writes conversation turns to `.t3x/conversations/<project>/conversation.jsonl`
- JSON Lines strictly conform to `schema/v1.0.json` (can reference `examples/*.t3x`): `id` uses `turn-<uuid>`, `role` is `user|assistant|system`, includes `text`, ISO 8601 `timestamp` fields, convenient for later analysis or synchronization
...
- `package.json`, `package-lock.json`: Define CLI metadata, execution entry `dist/bin/t3x.js`, dependencies and scripts
- `bin/t3x.ts`: CLI executable entry (shebang + main function)
- `src/runtime/t3xShell.ts`: Interactive shell main loop, handles chat/configuration dual modes
- `src/runtime/logger.ts`: Encapsulates log output with `[t3x]` prefix
- `src/core/`:
  - `config.ts`: Read/write `~/.config/t3x/config.json`, parse runtime configuration
  - `conversationStore.ts`: Manage `.t3x/conversations/<project>/conversation.jsonl`
  - `root.ts`: Find/create `.t3x` project root directory
...
- For debugging, run `npm start` or directly execute `node dist/bin/t3x.js`
