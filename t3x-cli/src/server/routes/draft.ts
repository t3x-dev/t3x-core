 ...
 */
async function getBridgeLoader(t3xDir: string): Promise<BridgeLoader> {
  const bridgesDir = path.join(t3xDir, "bridges");
 ...
 */
export function registerDraftRoutes(router: Router, providers: ProviderConfig, t3xDir: string): void {
  ...
  router.post("/api/v1/draft", async (ctx, _req, res) => {
    ...
    try {
      ...
      const loader = await getBridgeLoader(t3xDir);
