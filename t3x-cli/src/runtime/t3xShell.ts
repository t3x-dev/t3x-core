...
export async function startT3xShell(): Promise<void> {
  ...
  try {
    ...
    logger.info(`🚀 T3X API server: http://${embeddedInfo.host}:${embeddedInfo.port}`);
  } catch (error) {
  ...
  initProjectCache(storageRoot.t3xDir);
  try {
    await syncCache();
  ...
  let store = jsonlEnabled ? new ConversationStore(storageRoot.t3xDir, 'default') : null;
  ...
  try {
    while (true) {
      ...
      if (state.mode === 'config') {
        const result = await handleConfigMode(trimmed, state, store, storageRoot.t3xDir);
        if (result === 'back') {
          state.mode = 'chat';
          prompt = CHAT_PROMPT;
          rl.setPrompt(prompt);
      ...
      if (trimmed.startsWith('/')) {
        const slashHandled = await handleSlashCommand(trimmed, state, storageRoot.t3xDir, rl);
        if (slashHandled) {
          continue;
        }
        ...
        if (commandResult.type === 'switchProject') {
          ...
          store = jsonlEnabled ? new ConversationStore(storageRoot.t3xDir, state.project) : null;
          state.messages = [];
          await hydrateSessionMessages(store, state);
...
function logWelcomeBanner(model: string): void {
  ...
  const lines = [
    `╭${horizontal}╮`,
    formatBannerLine(`^_^ t3x  (${APP_VERSION})`),
    formatBannerLine(''),
