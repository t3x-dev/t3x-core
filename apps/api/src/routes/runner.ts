...
/**
 * GET /runner/suites - List available eval suites
 */
runnerRoutes.get('/runner/suites', (c) => {
  try {
...
/**
 * GET /runner/suites/:id - Get a specific suite
 */
runnerRoutes.get('/runner/suites/:id', (c) => {
  try {
...
/**
 * POST /runner/suites/:id/run - Run all cases in a suite
 */
runnerRoutes.post('/runner/suites/:id/run', async (c) => {
  try {
    const suiteId = c.req.param('id');
    ...
    const agent = observer.getAgent(suite.agent_id);
    if (!agent) {
      return c.json({ success: false, error: `Agent not found: ${suite.agent_id}` }, 404);
    }
    ...
    for (const evalCase of suite.cases) {
      try {
        ...
        // Run agent
        const runId = observer.startRun(suite.agent_id, {
          agent_id: suite.agent_id,
          input: inputPayload,
        });
        ...
        const evalResult = await evalEngine.evaluate({
          trace,
          test_steps: caseToTestSteps(evalCase),
          options: { stop_on_first_failure: false, generate_suggestions: true },
        });
        ...
        result.case_results.push(caseResult);
