...
test('discover real releases, filter, inspect an exact introduction and open Studio', async ({ page, request }, testInfo) => {
  ...
  try {
    ...
    await expect(page.getByRole('button', { name: 'Explore Team release definition 1.2.3' }).locator('img')).toBeVisible();
    ...
    await expect(page.getByRole('heading', { name: 'Browse schemas' })).toBeVisible();
    ...
    await expect(page.getByText('No schemas match these filters.', { exact: true })).toBeVisible();
    await page.getByRole('button', { name: 'Clear filters', exact: true }).click();
    await expect(page).toHaveURL((url) => !url.searchParams.has('tags') && !url.searchParams.has('ecosystem'));
    ...
    await expect(page.getByRole('heading', { name: 'Team release guide' })).toBeVisible();
    await page.getByRole('button', { name: 'Add & open Studio', exact: true }).click();
    await expect(page).toHaveURL(/schemaView=studio/);
    await expect(page.getByRole('checkbox', { name: 'Select Team release definition 1.2.3', exact: true })).toBeChecked();
    await page.screenshot({ path: testInfo.outputPath('studio-desktop.png'), animations: 'disabled' });
    await page.getByRole('button', { name: 'Advanced definition workbench' }).click();
