...
const png = 'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+a0uoAAAAASUVORK5CYII=';
...
test('author revision preserves State and history, renders uploaded content, and retains a stale draft', async ({ page, request }, testInfo) => {
  ...
  try {
    ...
    const first = await createTestCommitFromTrees(request, projectId, [{ key: 'service', slots: { image: 'app:v1' }, children: [] }]);
    ...
    await page.goto(`${projectPath}?view=overview&commit=${encodeURIComponent(first)}`, { waitUntil: 'networkidle' });
    ...
    await dialog.locator('input[type=file]').setInputFiles({ name: 'mark.png', mimeType: 'image/png', buffer: Buffer.from(png, 'base64') });
    ...
    await page.goto(`${projectPath}?view=overview&commit=${encodeURIComponent(revision)}`, { waitUntil: 'networkidle' });
    await page.getByRole('button', { name: 'Edit introduction', exact: true }).click();
