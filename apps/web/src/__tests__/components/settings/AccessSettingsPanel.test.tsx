...
describe('AccessSettingsPanel', () => {
  ...
  const openAccessCheck = {
    ...
    message: 'The target API is reachable and does not currently require a key.',
    api_url: 'http://localhost:8000/api',
    api_key_present: false,
    api_key_source: 'none' as const,
    status_code: 200,
  ...
  it('revokes a T3X API key and refreshes the list', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Revoke Old automation key' }));
  ...
  it('does not revoke a T3X API key when confirmation is cancelled', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Revoke Keep this key' }));
