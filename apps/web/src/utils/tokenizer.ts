...
export function getSelectionForToken(
  tokenIndex: number,
...
export function toggleKeyword(
  ...
  if (existing) {
    if (existing.constraint === constraint) {
      ...
      return keywords.map((kw) => (kw.tokenIndex === tokenIndex ? { ...kw, constraint } : kw));
    }
  } else {
    // Add new keyword
    return [
...
export function isTokenInIncludeSelection(
  tokenIndex: number,
  selections: TextSelection[]
): boolean {
  return selections.some(
    (sel) => sel.type === 'include' && tokenIndex >= sel.startIndex && tokenIndex <= sel.endIndex
  );
}

// Check if token is in an exclude selection
export function isTokenInExcludeSelection(
  tokenIndex: number,
  selections: TextSelection[]
): boolean {
  return selections.some(
    (sel) => sel.type === 'exclude' && tokenIndex >= sel.startIndex && tokenIndex <= sel.endIndex
  );
}
...
export function getSelectedText(tokens: TextToken[], selections: TextSelection[]): string {
  ...
  const selectedTokens = tokens.filter((token) =>
    isTokenInSelection(token.index, includeSelections)
  );
