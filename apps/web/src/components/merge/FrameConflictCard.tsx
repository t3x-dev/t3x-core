...
/**
 * FrameConflictCard — card for resolving a single frame-level merge conflict.
 *
 * Two modes:
 * - Per-frame (default): Side-by-side source/target YAML preview with
...
// ============================================================================
// SlotValue renderer (same syntax colors as CommitFrameCard / FrameDiffCard)
// ============================================================================
...
// ============================================================================
// FrameYamlPreview — compact YAML-style slot list for one frame side
// ============================================================================
...
function FrameYamlPreview({ frame, conflictKeys, label, labelColor }: FrameYamlPreviewProps) {
  return (
...
function PerSlotRow({ conflict, choice, frameId, onChoose }: PerSlotRowProps) {
  const radioName = `slot-${frameId}-${conflict.key}`;
...
export function FrameConflictCard({
  conflict,
  resolution,
  onResolve,
  isActive,
  onSelect,
}: FrameConflictCardProps) {
  const [mode, setMode] = useState<'per-frame' | 'per-slot'>('per-frame');
  ...
  const allSlotKeys = new Set([
    ...Object.keys(sourceFrame.slots),
    ...Object.keys(targetFrame.slots),
  ]);
  ...
  // Handlers for per-frame buttons
    ...
    >
      ...
      <div className="flex items-center justify-between gap-2 border-b border-[var(--stroke-divider)] px-4 pl-5 py-2">
        <div className="flex items-center gap-2 min-w-0">
          ...
          {/* Frame ID */}
          <span className="font-mono text-[11px] text-[var(--text-tertiary)]">{frameId}</span>
        </div>
