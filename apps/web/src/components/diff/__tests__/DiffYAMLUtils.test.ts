...
import { buildAlignedFrames, deriveRootFrameId, buildFrameTree, buildDiffStatusMap } from '../DiffYAMLUtils';

describe('buildAlignedFrames', () => {
  it('orders: modified -> removed -> added -> identical', () => {
    const diff: FrameDiff = {
      modified: [{ frameId: 'f1', sourceFrame: { id: 'f1', type: 'a', slots: { x: 1 } }, targetFrame: { id: 'f1', type: 'a', slots: { x: 2 } }, slotDiffs: [{ key: 'x', type: 'changed', oldValue: 1, newValue: 2 }] }],
  ...
  it('handles empty diff', () => {
    const diff: FrameDiff = { modified: [], onlyInSource: [], onlyInTarget: [], identical: [], relationsAdded: [], relationsRemoved: [] };
...
describe('deriveRootFrameId', () => {
  it('returns explicit root_frame_id if set', () => {
    const content: SemanticContent = {
      root_frame_id: 'f_002',
      frames: [{ id: 'f_001', type: 'a', slots: {} }, { id: 'f_002', type: 'b', slots: {} }],
  ...
  it('derives root from most incoming edges', () => {
    const content: SemanticContent = {
      frames: [{ id: 'f_001', type: 'a', slots: {} }, { id: 'f_002', type: 'b', slots: {} }, { id: 'f_003', type: 'c', slots: {} }],
      relations: [
...
describe('buildDiffStatusMap', () => {
  it('maps all frame statuses', () => {
    const diff: FrameDiff = {
      modified: [{ frameId: 'f1', sourceFrame: { id: 'f1', type: 'a', slots: {} }, targetFrame: { id: 'f1', type: 'a', slots: { x: 1 } }, slotDiffs: [] }],
      onlyInSource: [{ id: 'f2', type: 'b', slots: {} }],
      onlyInTarget: [{ id: 'f3', type: 'c', slots: {} }],
      identical: [{ id: 'f4', type: 'd', slots: {} }],
      relationsAdded: [],
...
describe('buildFrameTree', () => {
  it('builds tree from relations', () => {
    ...
    const statusMap = new Map<string, 'modified' | 'added' | 'removed' | 'identical'>([
      ['f_001', 'modified'], ['f_002', 'identical'], ['f_003', 'added'],
    ]);
