...
/**
 * FrameDiffCard — YAML-style card for a single frame in a frame-level diff.
 *
 * Four modes:
...
// ============================================================================
// SlotValue renderer (same syntax colors as CommitFrameCard)
// ============================================================================
...
// ============================================================================
// SlotRow — a single key:value line in a modified frame
// ============================================================================
...
export function FrameDiffCard({
  ...
  // Removed slot entries: present in sourceFrame but not in current frame (modified mode)
  const removedSlotEntries: Array<{ key: string; value: SlotValue }> = [];
  if (type === 'modified' && sourceFrame) {
    for (const key of Object.keys(sourceFrame.slots)) {
      if (!(key in frame.slots)) {
        removedSlotEntries.push({ key, value: sourceFrame.slots[key] });
      }
    }
  }
    ...
    >
      ...
      {!collapsed && (
        ...
        >
          ...
          {/* Removed slots (modified frames only — slots that existed in source but not target) */}
