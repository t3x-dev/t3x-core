...
function getScoreColorClasses(value: number): { bar: string; text: string } {
  if (value >= 0.7) {
    return { bar: 'bg-[var(--status-success)]', text: 'text-[var(--status-success)]' };
  }
  if (value >= 0.4) {
    return { bar: 'bg-[var(--status-warning)]', text: 'text-[var(--status-warning)]' };
  }
  return { bar: 'bg-[var(--status-error)]', text: 'text-[var(--status-error)]' };
}
