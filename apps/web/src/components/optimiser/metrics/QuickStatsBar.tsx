...
import type { EngineRun } from '@/types/api';
...
export function QuickStatsBar({ runs }: QuickStatsBarProps) {
  ...
  const latenciesWithValues = evalDataList
    .filter((d) => d.latencyMs !== null)
    .map((d) => d.latencyMs!);
  const avgLatencyMs =
    latenciesWithValues.length > 0
  ...
  return (
    <div className="grid gap-[var(--space-group)] sm:grid-cols-2 lg:grid-cols-4">
      <StatCard
        ...
        value={totalRuns}
        subValue={
          completedRuns.length < totalRuns ? `${completedRuns.length} completed` : undefined
        }
      />
      <StatCard
        ...
        subValue={
          completedRuns.length > 0
            ? `${Math.round((passedRuns / completedRuns.length) * 100)}% pass rate`
            : undefined
        }
      />
      <StatCard
      ...
      <StatCard
        ...
        subValue={
          latenciesWithValues.length > 0 ? `from ${latenciesWithValues.length} runs` : undefined
        }
      />
    </div>
  );
