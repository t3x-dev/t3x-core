...
import { gateCheck } from '@/lib/api/trees';
...
interface GateQualityTabProps {
  ...
  onLocateNode?: (treeId: string) => void;
  onSwitchToTree?: () => void;
...
export function GateQualityTab({
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full text-muted-foreground p-4">
        ...
        <p className="text-xs text-center mt-1">
          Quality checks verify that extracted trees accurately reflect the conversation.
        </p>
      </div>
    );
  }
  ...
  const handleLocate = (treeId: string) => {
    onLocateNode?.(treeId);
    onSwitchToTree?.();
  };
      ...
      <div className="p-3 space-y-4 flex-1">
        ...
        {issues.length > 0 && (
          <div className="space-y-2">
            ...
            {issues.map((issue, i) => (
              <GateIssueCard
                key={`${issue.dimension}-${issue.tree_id ?? i}`}
                issue={issue}
