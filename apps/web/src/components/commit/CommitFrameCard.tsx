...
/**
 * CommitFrameCard — YAML-style card for a single semantic frame on the commit detail page.
 *
 * Features:
 * - Header bar: type badge, frame ID, diff status label, confidence
...
export interface CommitFrameCardProps {
  enrichedFrame: EnrichedFrame;
  isActive: boolean;
  onSelect: () => void;
...
function computeSlotDiff(frame: Frame, previousFrame: Frame | undefined): SlotDiffSets {
  ...
  const prevKeys = new Set(Object.keys(previousFrame.slots));
  const currKeys = new Set(Object.keys(frame.slots));

  for (const key of currKeys) {
    if (!prevKeys.has(key)) {
      addedSlots.add(key);
    } else if (JSON.stringify(frame.slots[key]) !== JSON.stringify(previousFrame.slots[key])) {
  ...
  for (const key of prevKeys) {
    if (!currKeys.has(key)) {
      removedSlotEntries.push({ key, value: previousFrame.slots[key] });
    }
  }
...
export function CommitFrameCard({ enrichedFrame, isActive, onSelect, cardRef }: CommitFrameCardProps) {
  ...
  // Compute slot-level diff (only meaningful for 'modified' frames)
  const { changedSlots, addedSlots, removedSlotEntries } = computeSlotDiff(
    frame,
    diffStatus === 'modified' ? previousFrame : undefined
  );
    ...
    >
      ...
      >
        <div className="flex items-center gap-2 min-w-0">
          ...
          {/* Frame ID */}
          <span className="font-mono text-[11px] text-[var(--text-tertiary)]">{frame.id}</span>
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          {frame.confidence != null && (
            <ConfidenceBadge value={frame.confidence} pulse={isActive} />
          )}
        </div>
      </div>
      ...
      <div className="px-4 pl-5 py-3 space-y-0.5 bg-[var(--surface-app)]/40">
        ...
        {Object.entries(frame.slots).map(([key, value]) => (
          <SlotRow
            ...
            oldValue={changedSlots.has(key) && previousFrame ? previousFrame.slots[key] : undefined}
        ...
        {/* Removed slots (only appear in modified frames) */}
