...
export const bridgeQueryDefs: Record<BridgeTemplate, { task: string; schema: string }> = {
  summary: {
    task:
      'Extract the most important conclusions and factual takeaways: key numbers, core judgments, and summary statements. Prefer high-signal content; ignore detailed elaboration and repetition.',
    schema:
  ...
  plan: {
    task:
      'Extract actionable steps and tasks: order, dependencies, prerequisites, roles, and acceptance criteria. Prefer list-like, instruction-style content.',
    schema:
      'Output numbered steps. Each step includes: Goal -> Action -> Expected outcome / Acceptance criteria.',
  },
  prose: {
    task:
      'Extract paragraph-ready content: definitions, explanations, reasons, contrasts, reasoning chains, viewpoints, and implications. Prefer logically complete statements.',
    schema:
  ...
  story: {
    task:
      'Extract narrative elements: timeline events, causality, characters/setting, conflict and resolution. Prefer content that supports a coherent storyline.',
    schema:
      'Preferred arc: setup -> development -> climax -> resolution. Keep transitions and connections; avoid jumpy isolated quotes.',
  },
  refine: {
    task:
      'Extract two kinds of sentences: (1) core sentences that must be preserved (key info/conclusions); (2) sentences that need rewriting (unclear, redundant, illogical flow, inconsistent style). Prefer sentence-level granularity.',
    schema:
  ...
  explain: {
    task:
      'Extract content that supports a clear explanation: definitions, reasoning steps, examples, and clarifications that reduce confusion.',
    schema:
      'Explain in a structured way: concept -> why it matters -> how it works -> example (optional). Keep it clear and concise.',
  },
  clarify: {
    task:
      'Extract ambiguous points, missing assumptions, and places where the intent is unclear. Prefer content that would benefit from asking clarifying questions.',
    schema:
