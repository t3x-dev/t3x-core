...
export function traceYamlToChat(
  ...
  if (hoveredSlotKey) {
    // Specific slot — look up by key (with and without path prefix)
    const quote =
  ...
  // Node header — collect all quotes under this path
  const prefix = stripRoot(dotPath);
  const matchingQuotes = Object.entries(allQuotesMap)
    .filter(([key]) =>
      prefix ? key.startsWith(`${prefix}.`) || key === prefix : !key.includes('.')
    )
    .map(([, value]) => value);
 ...
 */
export function traceChatToYaml(draft: SemanticContent, turnIndex: number): string[] {
